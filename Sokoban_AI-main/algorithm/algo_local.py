...
def simulated_annealing_search(game, initial_temperature=1000, cooling_rate=0.95, max_iterations=1000):
    ...
    # Annealing is stochastic; use a wider local beam as a stable recovery path
    # so the UI can still compare a complete route after an unlucky run.
    print("Annealing did not converge; switching to local beam recovery.")
    return beam_best_first(game, beam_width=12, max_depth=500)
